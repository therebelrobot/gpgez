...
function _utilEmailToKey(email, keys, cb) {
  ...
  if (matching.length > 1) {
    ...
    debug('No key found matching that email address')
    cb()
  } else {
    debug('Found one result')
    cb(matching[0])
  }
}
  ...
  var allKeys = _.map(keyArray, function _parseKeyMap(key, index) {
    ...
    if (key.length > 1) {
      ...
      var uids = _.map(key, function _parseKeyMapUidMap(line) {
        debug('_parseKeyMapUidMap entered')
        if (line.match(/^uid*.*/)) {
          line = line.split('uid ')
          line.shift()
          if(line.length == 1){
      ...
      sigs = _.map(key, function _parseKeyMapSigMap(line) {
        debug('_parseKeyMapSigMap entered')
        if (line.match(/^sig*.*/)) {
          var sig = {}
      ...
      var subs = _.map(key, function _parseKeyMapSubMap(line) {
        debug('_parseKeyMapSubMap entered')
        if (line.match(/^sub*.*/)) {
          var sub = {}
