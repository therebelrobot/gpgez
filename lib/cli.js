...
function _taskList() {
  ...
  _.forEach(keys, function(key,index){
    if(key.type){
      if (key.type === 'secret'){
        table.push(['\t',key.type.blue,key.date.blue.bold,key.firstName.blue,key.lastName.blue,key.comment.blue,key.email.blue,key.id.bold.blue])
      }
      else{
        table.push(['\t',key.type.green,key.date.gray.bold,key.firstName.gray,key.lastName.gray,key.comment.gray,key.email,key.id.bold.green])
      }
    }
  })
  console.log(table.toString())
